...
def all_supplies_in_holidays(holiday_hash)
  ...
  holiday_hash.each do |season_key, holiday_hash_value|
    ...
    holiday_hash_value.each do |holiday_key, supply_array_value|
      ...
      holiday_string_array = holiday_string_array.collect {|element| element}
      holiday_string = holiday_string_array.join.gsub('_',' ')
      holiday_caps = holiday_string.split.map(&:capitalize).join(' ')
      puts "  #{holiday_caps}: #{supply_array_value.join(', ')}"
    end
  end
end
